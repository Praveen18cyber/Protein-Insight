          ...
          return (
            <div key={idx} className="border border-border rounded-lg bg-white overflow-hidden">
              <button
                ...
                data-testid={`button-chain-pair-${idx}`}
                ...
                <div className="flex items-center gap-3 flex-1 text-left">
                  ...
                  <div className="flex gap-4 text-xs">
                    <div className="text-right">
                      ...
                      <div className="font-semibold text-foreground" data-testid={`text-chain-total-${idx}`}>{item.totalCount}</div>
                    </div>
                  </div>
                </div>
                <ChevronDown
              ...
              {isExpanded && (
                <div className="px-4 py-3 bg-muted/30 border-t border-border grid grid-cols-2 gap-4 text-sm">
                  <div>
                    ...
                    <div className="text-lg font-bold text-orange-600" data-testid={`text-chain-intra-${idx}`}>{item.intraCount}</div>
                  </div>
                  <div>
                    <div className="text-xs text-muted-foreground font-semibold mb-1">Inter-Chain</div>
                    <div className="text-lg font-bold text-blue-600" data-testid={`text-chain-inter-${idx}`}>{item.interCount}</div>
                  </div>
                </div>
              )}
            </div>
