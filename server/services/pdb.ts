...
// RCSB PDB GraphQL API Endpoint
const RCSB_API_URL = "https://data.rcsb.org/graphql";

export async function fetchPDBMetadata(pdbId: string) {
  const query = `
    query($id: String!) {
      entry(entry_id: $id) {
        rcsb_entry_info {
  ...
  try {
    const response = await axios.post(RCSB_API_URL, {
      query,
      variables: { id: pdbId.toUpperCase() }
    });
    ...
    const method = entry.exptl?.[0]?.method || "N/A";
    const releaseDate = entry.rcsb_accession_info?.initial_release_date;
    const year = releaseDate ? new Date(releaseDate).getFullYear() : null;

    const chains = (entry.polymer_entities || []).flatMap((entity: any) => {
      const organism = entity.rcsb_entity_source_organism?.[0]?.scientific_name || "N/A";
      const uniprot = entity.rcsb_polymer_entity_container_identifiers?.uniprot_ids?.[0] || null;

      const chainIds = entity.rcsb_polymer_entity_container_identifiers?.auth_asym_ids || [];
      return chainIds.map((id: string) => ({
        id,
        organism,
        uniprot,
    ...
    return {
      method,
      resolution,
      organism: chains?.[0]?.organism || "N/A",
      year,
      chains
    };
  } catch (error) {
...
// 7. Binding Affinity Index (BAI) Calculation
// Formula: (1.5 × H-Bond) + (2.0 × Salt Bridge) + (1.0 × Hydrophobic) + (0.2 × VdW) - (0.5 × Avg Distance)
export function calculateBindingAffinityIndex(result: AnalysisResult) {
  const interInteractions = result.interactions.filter(i => !i.isIntraMolecular);
  if (interInteractions.length === 0) return null;

  const counts = {
    hydrogenBonds: interInteractions.filter(i => i.type === "Hydrogen Bond").length,
    saltBridges: interInteractions.filter(i => i.type === "Salt Bridge").length,
    hydrophobicContacts: interInteractions.filter(i => i.type === "Hydrophobic").length,
    vanDerWaals: interInteractions.filter(i => i.type === "Van der Waals").length,
  };

  const avgDistance = interInteractions.reduce((sum, i) => sum + i.distance, 0) / interInteractions.length;

  const rawScore =
    (1.5 * counts.hydrogenBonds) +
    (2.0 * counts.saltBridges) +
    (1.0 * counts.hydrophobicContacts) +
    (0.2 * counts.vanDerWaals) -
    (0.5 * avgDistance);

  // Normalization heuristic: clamp to 0-100 for visual range
  // Strong interactions usually have scores > 50 in this model
  const normalized = Math.min(100, Math.max(0, Math.round(rawScore * 2)));

  let category = "Weak";
  if (normalized > 70) category = "Strong";
  else if (normalized > 40) category = "Moderate";

  return {
    bindingAffinityIndex: normalized,
    bindingCategory: category,
    featureContributions: {
      ...counts,
      distancePenalty: -Math.round(0.5 * avgDistance * 10) / 10
    }
  };
}
